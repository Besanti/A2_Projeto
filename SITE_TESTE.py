...
if jogador in desempenho:
    ...
    if corrected_jogador:
        ...
        for index, row in enumerate(linhas):
            ...
            for cell in cells:
                if cell.get_text().strip().lower() == "data de nascimento":
                    st.write("Data de Nascimento: " + cells[1].get_text())
                elif cell.get_text().strip().lower() == "nacionalidade":
                    st.write("Nacionalidade: " + cells[1].get_text())
                elif cell.get_text().strip().lower() == "local de nascimento":
                    st.write("Local de Nascimento: " + cells[1].get_text())
                elif cell.get_text().strip().lower() == "altura":
                    st.write("Altura: " + cells[1].get_text())
                elif cell.get_text().strip().lower() == "pé":
                    st.write("Pé preferido: " + cells[1].get_text())

        if corrected_jogador in extras:
            st.write(extras[corrected_jogador]["diga"])

    else:
        st.write(f"Não consegui encontrar informações sobre o jogador {jogadorao}. Verifique se o nome está correto.")

else:
    st.write(f"Desculpe, não tenho informações sobre o jogador {jogadorao}.")
